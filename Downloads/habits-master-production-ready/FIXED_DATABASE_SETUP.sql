-- FIXED DATABASE SETUP FOR FAITHHABITS APP
-- This version handles existing tables and column conflicts properly
...
DROP TABLE IF EXISTS spiritual_milestones CASCADE;
DROP TABLE IF EXISTS christian_habit_completions CASCADE;
DROP TABLE IF EXISTS christian_habits CASCADE;
DROP TABLE IF EXISTS christian_habit_templates CASCADE;
DROP TABLE IF EXISTS christian_books CASCADE;
DROP TABLE IF EXISTS service_records CASCADE;
DROP TABLE IF EXISTS worship_sessions CASCADE;
DROP TABLE IF EXISTS fasting_records CASCADE;
...
CREATE TABLE christian_habit_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
  category TEXT,
...
CREATE TABLE christian_habit_completions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_fasting_records_user_id ON fasting_records(user_id);
CREATE INDEX idx_worship_sessions_user_id ON worship_sessions(user_id);
CREATE INDEX idx_service_records_user_id ON service_records(user_id);
CREATE INDEX idx_christian_books_user_id ON christian_books(user_id);
CREATE INDEX idx_christian_habits_user_id ON christian_habits(user_id);
CREATE INDEX idx_spiritual_milestones_user_id ON spiritual_milestones(user_id);
...
CREATE POLICY "Users can view own practice logs" ON practice_logs FOR SELECT USING (auth.uid() = (SELECT user_id FROM practices WHERE id = practice_id));
CREATE POLICY "Users can insert own practice logs" ON practice_logs FOR INSERT WITH CHECK (auth.uid() = (SELECT user_id FROM practices WHERE id = practice_id));
...
CREATE POLICY "Users can view own habit completions" ON christian_habit_completions FOR SELECT USING (auth.uid() = (SELECT user_id FROM christian_habits WHERE id = habit_id));
CREATE POLICY "Users can insert own habit completions" ON christian_habit_completions FOR INSERT WITH CHECK (auth.uid() = (SELECT user_id FROM christian_habits WHERE id = habit_id));
