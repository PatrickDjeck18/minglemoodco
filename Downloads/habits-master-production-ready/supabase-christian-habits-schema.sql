-- =====================================================
-- SUPABASE CHRISTIAN HABITS TRACKING SCHEMA
...
-- Prayer requests with enhanced features
...
-- Prayer sessions with detailed tracking
...
INSERT INTO christian_habit_templates (name, description, category, frequency, suggested_duration, scripture_reference, benefits, difficulty_level, is_default) VALUES
...
ALTER TABLE prayer_sessions ADD CONSTRAINT check_duration_positive 
  CHECK (duration > 0);

ALTER TABLE service_records ADD CONSTRAINT check_duration_positive 
...
CREATE INDEX IF NOT EXISTS idx_prayer_requests_user_private ON prayer_requests(user_id, is_private);
...
DO $$
BEGIN
  RAISE NOTICE 'Christian Habits Tracking Schema Successfully Created!';
