...
const PracticesScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const activePractices = [
    ...
    },
    {
      ...
      color: '#2DCE89',
      icon: 'leaf.fill',
      lastCompleted: new Date(Date.now() - 172800000),
    },
  ];
  ...
  const handlePracticeCheckIn = async (practiceId: number) => {
    try {
      if (user) {
        await FirebaseManager.updatePracticeCompletion(practiceId.toString(), true);
        console.log('Practice completion updated in Firebase');
      }
  ...
  const handleCreatePractice = async () => {
    if (newPracticeName.trim() && selectedCategory) {
      try {
        if (user) {
          await FirebaseManager.savePractice(user.uid, {
            name: newPracticeName.trim(),
            category: selectedCategory,
            frequency: 'daily',
            goal: 30,
          });
          console.log('Practice saved to Firebase');
        }
